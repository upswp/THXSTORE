      ...
      <article v-for="(order, index) in orderList" :key="index" class="order-item">
        <header class="order-header">
          ...
          <main class="item-info">
            <div class="item-store">{{ order.storeName }}</div>
            <div class="item-date">{{ dateTrans(order.orderTime) }}</div>
            <div class="item-status">{{ orderStatusStrConvert[order.reservationStatus] }}</div>
          </main>
        </header>
        <div class="order-menu-container">
          <div v-for="(menu, menuOrder) in order.reservationGroups" :key="menuOrder" class="order-body">
            <div class="item-name">
              {{ menu.productName }} <small>{{ menu.count }}개</small>
            </div>
            <div class="item-price">{{ wonTrans(menu.computed) }}</div>
          </div>
        </div>
        <div class="order-accum-price">
          <div class="accum-price-label"><b>총 결제금액</b></div>
          <div class="accum-price">
            <b>{{ wonTrans(order.totalPayment) }}</b>
          </div>
        </div>
        <div v-if="statusCheck(order.reservationStatus)" class="order-footer" @click="toggleReviewLoaded(index)">
